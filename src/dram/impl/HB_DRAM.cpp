...
namespace Ramulator {
// using DDR4's base structures
class HB_DRAM : public IDRAM, public Implementation {
  RAMULATOR_REGISTER_IMPLEMENTATION(IDRAM, HB_DRAM, "HB_DRAM", "Hybrid Bonding DRAM Device Model")

  public:
    inline static const std::map<std::string, Organization> org_presets = {
      // 这里的 Bg (Bank Group) -> Bank (2MB)
      // 这里的 Ba (Bank)       -> Subarray
      // 假设1个subarray
      //   name      density   DQ    Ch  Ra  Bg  Ba   Ro     Co
      {"HB_16Mb_x64", {16,     64, { 1,  1,  1,  1, 1<<12, 1<<6}}},
      {"HB_1Gb_x64",  {1<<10,  64, { 1,  1,  1, 64, 1<<12, 1<<6}}}, 
    };

    inline static const std::map<std::string, std::vector<int>> timing_presets = {
      // rate: 500 (500MHz) -> 不是DDR，1个时钟周期传输64bit数据
  ...
  public:
    struct Node : public DRAMNodeBase<HB_DRAM> {
      Node(HB_DRAM* dram, Node* parent, int level, int id) : DRAMNodeBase<HB_DRAM>(dram, parent, level, id) {};
    };
  ...
  private:
    ...
    void set_timing_vals() {
      ...
      // [从双边沿 (DDR) 改为 单边沿 (SDR) 逻辑
      // 使用 HB (Hybrid Bonding) Preset，不再除以 2
      int tCK_ps;
      // SDR: 500 rate = 500 MHz = 2000ps
      tCK_ps = 1E6 / m_timing_vals("rate");
      m_timing_vals("tCK_ps") = tCK_ps;

      // Load the organization specific timings (keeping lookup tables for legacy DDR4 support)
      ...
      // Default calculation (only if not set in preset)
      if (m_timing_vals("nRFC") == -1) {
         // Default to smallest density index 0 for safety if using non-standard density
         m_timing_vals("nRFC")  = JEDEC_rounding(tRFC_TABLE[0][0], tCK_ps);
      }
      if (m_timing_vals("nREFI") == -1) {
      ...
      populate_timingcons(this, {
          /*** Channel ***/ 
          // [User Mod] nBL=1 (SDR 64bit)，所以总线占用就是 1 cycle
          ...
          // [User Mod] Interpretation:
          // nCCDS = Column-to-Column Delay Short (Same BankGroup)
          // Mapped to: Same Physical Bank (2MB), Different Subarray.
          ...
          // [User Mod] nRRDS = Activate to Activate (Different BG) -> 不同 2MB 物理 Bank
          ...
          /*** Same Bank Group ***/ 
          // [User Mod] Mapped to: Same Physical Bank (2MB)
          // nCCDL = Column-to-Column Delay Long (Different BG) -> 其实这里原来的DDR4逻辑是 nCCDL 用于 SameBG (不，DDR4逻辑是 SameBG用nCCDL? 不，Standard DDR4 is nCCDL for Same BG? Wait.)
          // Check: DDR4 Standard. 
          // tCCD_S: Different Bank Group (Short delay)
          // tCCD_L: Same Bank Group (Long delay)
          // Ramulator Logic above: 
          // rank level (implied diff BG) uses nCCDS.
          // bankgroup level (implied same BG) uses nCCDL.
          // 所以：
          // nCCDS = 不同 BankGroup (Physical Bank) 之间的延迟 (应较小)
          // nCCDL = 相同 BankGroup (Physical Bank) 内部，不同 Bank (Subarray) 之间的延迟 (应较大或相等)
          
          // 在 3D IC 中，通常我们希望 Subarray 切换也很快，所以建议 nCCDL = nCCDS = 1
          ...
          // nRRDL = Act-Act Same BG (Inside same Physical Bank, different Subarray)
          {.level = "bankgroup", .preceding = {"ACT"}, .following = {"ACT"}, .latency = V("nRRDL")},  

          /*** Bank ***/ 
          // [User Mod] Mapped to: Subarray Level constraints
    ...
    void set_actions() {
      ...
      // Rank Actions
      m_actions[m_levels["rank"]][m_commands["PREA"]] = Lambdas::Action::Rank::PREab<HB_DRAM>;
      m_actions[m_levels["rank"]][m_commands["REFab"]] = Lambdas::Action::Rank::REFab<HB_DRAM>;
      m_actions[m_levels["rank"]][m_commands["REFab_end"]] = Lambdas::Action::Rank::REFab_end<HB_DRAM>;

      // Bank actions
      m_actions[m_levels["bank"]][m_commands["ACT"]] = Lambdas::Action::Bank::ACT<HB_DRAM>;
      m_actions[m_levels["bank"]][m_commands["PRE"]] = Lambdas::Action::Bank::PRE<HB_DRAM>;
      m_actions[m_levels["bank"]][m_commands["RDA"]] = Lambdas::Action::Bank::PRE<HB_DRAM>;
      m_actions[m_levels["bank"]][m_commands["WRA"]] = Lambdas::Action::Bank::PRE<HB_DRAM>;
    };
    ...
    void set_preqs() {
      ...
      // Rank Actions
      m_preqs[m_levels["rank"]][m_commands["REFab"]] = Lambdas::Preq::Rank::RequireAllBanksClosed<HB_DRAM>;

      // Bank actions
      m_preqs[m_levels["bank"]][m_commands["RD"]] = Lambdas::Preq::Bank::RequireRowOpen<HB_DRAM>;
      m_preqs[m_levels["bank"]][m_commands["WR"]] = Lambdas::Preq::Bank::RequireRowOpen<HB_DRAM>;
      m_preqs[m_levels["bank"]][m_commands["ACT"]] = Lambdas::Preq::Bank::RequireRowOpen<HB_DRAM>;
      m_preqs[m_levels["bank"]][m_commands["PRE"]] = Lambdas::Preq::Bank::RequireBankClosed<HB_DRAM>;
    };

    void set_rowhits() {
      m_rowhits.resize(m_levels.size(), std::vector<RowhitFunc_t<Node>>(m_commands.size()));

      m_rowhits[m_levels["bank"]][m_commands["RD"]] = Lambdas::RowHit::Bank::RDWR<HB_DRAM>;
      m_rowhits[m_levels["bank"]][m_commands["WR"]] = Lambdas::RowHit::Bank::RDWR<HB_DRAM>;
    }
    ...
    void set_rowopens() {
      ...
      m_rowopens[m_levels["bank"]][m_commands["RD"]] = Lambdas::RowOpen::Bank::RDWR<HB_DRAM>;
      m_rowopens[m_levels["bank"]][m_commands["WR"]] = Lambdas::RowOpen::Bank::RDWR<HB_DRAM>;
    }
    ...
    void set_powers() {
      ...
      m_powers[m_levels["bank"]][m_commands["ACT"]] = Lambdas::Power::Bank::ACT<HB_DRAM>;
      m_powers[m_levels["bank"]][m_commands["PRE"]] = Lambdas::Power::Bank::PRE<HB_DRAM>;
      m_powers[m_levels["bank"]][m_commands["RD"]]  = Lambdas::Power::Bank::RD<HB_DRAM>;
      m_powers[m_levels["bank"]][m_commands["WR"]]  = Lambdas::Power::Bank::WR<HB_DRAM>;

      m_powers[m_levels["rank"]][m_commands["ACT"]] = Lambdas::Power::Rank::ACT<HB_DRAM>;
      m_powers[m_levels["rank"]][m_commands["PRE"]] = Lambdas::Power::Rank::PRE<HB_DRAM>;
      m_powers[m_levels["rank"]][m_commands["PREA"]] = Lambdas::Power::Rank::PREA<HB_DRAM>;
      m_powers[m_levels["rank"]][m_commands["REFab"]] = Lambdas::Power::Rank::REFab<HB_DRAM>;
      m_powers[m_levels["rank"]][m_commands["REFab_end"]] = Lambdas::Power::Rank::REFab_end<HB_DRAM>;
    ...
    void process_rank_energy(PowerStats& rank_stats, Node* rank_node) {
      ...
      Lambdas::Power::Rank::finalize_rank<HB_DRAM>(rank_node, 0, AddrVec_t(), m_clk);
