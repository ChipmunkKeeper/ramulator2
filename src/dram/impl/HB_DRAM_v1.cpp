...
class DDR4 : public IDRAM, public Implementation {
  ...
  public:
    inline static const std::map<std::string, Organization> org_presets = {
      ...
      // 特点：DQ=64, BankGroup(Bg)=1 (移除BG层级影响), Bank(Ba)=16 (假设16个Bank)
      {"HB_16Mb_x64",    {16,     64, {1, 1, 1, 1, 1<<14, 1<<6}}}, 
    };
    ...
    inline static const std::map<std::string, std::vector<int>> timing_presets = {
      ...
      // [User Mod] 新增 3D Hybrid Bonding Timing
      // Rate: 500 (代表 500MHz，将在 set_timing_vals 中特殊处理)
      // nBL: 1 (Burst Length = 1 cycle, 每次传输64bit)
      // nCCDS/L: 1 (允许每个周期连续发送列命令，前提是不同Bank，或者同Bank视具体设计而定)
      // nRCD/nRP: 3D堆叠通常延迟极低，这里假设较小值，例如 5 cycles (10ns)，可根据你的仿真数据修改
      // nRAS: Row Active Time, 假设 10 cycles
      // nRC:  RC = RAS + RP = 15
      // nRRDS/L: Activate to Activate delay, 设为极小值 2
      // nFAW: Four Activate Window, 设为 8 (或者更小，取决于功耗限制)
      {"HB_500_SDR",  {500,    1,   5,   5,    5,    10,   15,   5,    2,   5,    1,    1,    2,    2,    1,    1,    8,   -1,  -1,   0,    2000}},
    };

    inline static const std::map<std::string, std::vector<double>> voltage_presets = {
      //   name          VDD      VPP
      {"Default",       {1.2,     2.5}},
      {"HB_LowPower",   {1.0,     1.8}}, // [User Mod] 3D IC 可能电压更低
    };
  ...
  public:
    ...
    void init() override {
      ...
      set_organization();
      set_timing_vals(); // 这里包含了对 HB 模式的特殊处理
  ...
  private:
    void set_organization() {
      ...
      // [User Mod] 这里的检查对于 HB 可能需要调整，只要 density 大致匹配即可
    ...
    void set_timing_vals() {
      ...
      // [User Mod] 关键修改：处理时钟频率计算 (tCK)
      // 如果 Preset 名字包含 "HB" (Hybrid Bonding) 或 "SDR"，我们假设 rate 就是时钟频率(MHz)
      // 否则，沿用 DDR 逻辑 (时钟频率 = rate / 2)
      int tCK_ps;
      if (preset_name_str.find("HB") != std::string::npos || preset_name_str.find("SDR") != std::string::npos) {
          // SDR 模式：rate = Frequency (MHz)
          tCK_ps = 1E6 / m_timing_vals("rate");
      } else {
          // DDR 模式 (Default)：rate = MT/s = 2 * Frequency
          tCK_ps = 1E6 / (m_timing_vals("rate") / 2);
      }
      m_timing_vals("tCK_ps") = tCK_ps;

      // ... (省略了原始代码中查表计算 nRRDS, nFAW 等部分，因为 HB 模式下我们已经在 Preset 里直接指定了数值)
      // 为了保持代码简洁，这里保留原表查询逻辑，但你可以通过在 yaml 覆盖或者在 preset 中填入非-1值来跳过查表

      // [Refresh Calculation - Keep standard or override]
      ...
      // 仅当 preset 中未指定时才去计算 (DDR4 默认逻辑)
      if (m_timing_vals("nRFC") == -1) {
          int density_id = 0; // 简化处理，默认最小
      ...
      // Check for uninitialized timings
      ...
      // Populate timing constraints
      #define V(timing) (m_timing_vals(timing))
      populate_timingcons(this, {
          /*** Channel ***/ 
          // [User Mod] nBL=1 时，总线占用只有1个周期，完美适配你的 "每周期读一次"
          ...
          /*** Rank ***/ 
          // nCCDS/L 设为 1 后，这里的限制解除，变为真正的随机访问吞吐量限制
          ...
          /*** Same Bank Group ***/ 
          // [User Mod] 当 Bg=1 时，这些约束与 Rank 层的 nCCDS 重叠，只要 nCCDL = nCCDS = 1 即可
